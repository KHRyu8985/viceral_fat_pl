...
from lightning.pytorch.callbacks import BatchSizeFinder, LearningRateFinder, StochasticWeightAveraging
    ...
    train_split = [
        {
            ...
            "seg": os.path.join(base_dir, entry, "torso_fat.nii.gz"),
        }
...
class FatSegmentModel(pytorch_lightning.LightningModule):
    ...
    def __init__(self, arch_name='UNETR',batch_size=2, lr=1e-3, patch_size=(96, 96, 96)):
        super().__init__()
        ...
        if arch_name == 'SwinUNETR':
            ...
            swin_vit_weights = {k: v for k, v in weight.items() if k.startswith("swinViT")}
    ...
    def prepare_data(self):
        ...
        train_files, val_files, test_files = load_data_splits(yaml_path='data/KU-PET-CT/data_splits.yaml')
        ...
        train_transforms = Compose(
            [
                ...
                CropForegroundd(
                    keys=["image", "label", "seg"], source_key="image"
                ),
                RandCropByPosNegLabeld(
                    keys=["image", "label", "seg"],
                    label_key="label",
    ...
    def training_step(self, batch, batch_idx):
        images, labels, segs = batch["image"], batch["label"], batch["seg"]  # Include seg
        output = self.forward(images, segs)  # Pass seg to forward
        loss = self.loss_function(output, labels)
        metrics = loss.item()
        self.log("train_loss", metrics, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        outputs = sliding_window_inference(
            ...
            lambda x: self.forward(x[:, :1, ...], x[:, 1:, ...])  # Split before forward
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Save results
        self.save_results(images, outputs[0], labels[0], filename_prefix=f'batch_{batch_idx}')
...
@click.option(
    '--arch_name',
    type=click.Choice(['UNet', 'AttentionUnet', 'SegResNet', 'UNETR', 'SwinUNETR', 'VNet']),
    default='UNETR',
...
@click.option(
    ...
    type=int,
    default=0,
    help='Set the GPU index to use for training.'
)
@click.option(
    '--checkpoint_path',
    type=str,
...
def main(arch_name, max_epochs, check_val_every_n_epoch, gpu_number, checkpoint_path):
    # set up loggers and checkpoints
    log_dir = f"results/res_{arch_name}"
    ...
    # Set up callbacks
    callbacks = [StochasticWeightAveraging(swa_lrs=[1e-4], annealing_epochs=5, swa_epoch_start=100)]
    ...
    if checkpoint_path:
        ...
        net = FatSegmentModel.load_from_checkpoint(checkpoint_path, arch_name=arch_name)
        ...
        net = FatSegmentModel(arch_name=arch_name, batch_size=1)
        net.results_folder = Path(log_dir)  # Set results folder path
