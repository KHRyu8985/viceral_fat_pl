...
class FatSegmentModel(pytorch_lightning.LightningModule):
    ...
    def __init__(
        ...
        if arch_name == "UNet":
            ...
            self._model = UNETR(
                in_channels=1, out_channels=2, img_size=patch_size, feature_size=16
            )
...
@click.option("--fold_number", type=int, default=4, help="Specify the fold number for training.")
def main(arch_name, loss_fn, max_epochs, check_val_every_n_epoch, gpu_number, checkpoint_path, fold_number):
    # set up loggers and checkpoints
    log_dir = f"results/conven_{arch_name}_{fold_number}"
    ...
    if checkpoint_path is not None and os.path.exists(checkpoint_path):
        ...
        net = FatSegmentModel.load_from_checkpoint(checkpoint_path, arch_name=arch_name, loss_fn=loss_fn)
